...
这是一套构建在原版 Playwright 内核之上的 JS 注入 + CDP 层。它能通过常见的检测站点（CreepJS、browserleaks、pixelscan、iphey），并且确实做到了轻量。它**不会**像商业指纹浏览器那样去改浏览器的 C++ 源码——那条路是数小时、数 GB 的编译工作，不在本项目范围内。WebKit 配置始终呈现为 macOS 版 Safari（Safari 只在苹果平台上发行；Windows + WebKit 的组合本身就不自洽）。
